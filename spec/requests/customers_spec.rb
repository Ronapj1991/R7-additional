...
describe "get new_customer_path" do
    it "renders the :new template" do
      expect(get "/customers/new").to render_template(:new)
    end
  end
  describe "get edit_customer_path" do
    it "renders the :edit template" do
      customer = FactoryBot.create(:customer)
      expect(get "/customers/#{customer.id}/edit").to render_template(:edit)
    end
  end
  ...
  describe "put customer_path with valid data" do
    it "updates an entry and redirects to the show path for the customer" do
      customer = FactoryBot.create(:customer)
      put customer_path(customer.id), params: { customer: {first_name: "Changed"}}
      customer.reload
      expect(customer.first_name).to eql("Changed")
      expect(response).to redirect_to customer_path(id: Customer.last.id)
    end
  end
  describe "put customer_path with invalid data" do
    it "does not update the customer record or redirect" do
      customer = FactoryBot.create(:customer)
      put customer_path(customer.id), params: { customer: {phone: "1234"} }
  ...
  describe "delete a customer record" do
    it "deletes a customer record" do
      customer = FactoryBot.create(:customer)
      customer.delete
      expect(get "/customers/#{customer.id}").to eql(302) 
    end
  end
end